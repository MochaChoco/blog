...
SSR은 서버에서 미리 HTML을 생성하여 클라이언트에 전송하는 방식입니다. 이는 초기 로딩 속도 개선과 SEO에 매우 유리합니다. 브라우저는 서버로부터 받은 완전한 HTML을 즉시 렌더링할 수 있기 때문에, 사용자는 콘텐츠를 더 빠르게 볼 수 있습니다.
...
SCSS Mixin은 재사용 가능한 CSS 선언 그룹을 정의하는 기능입니다. `@mixin`으로 정의하고 `@include`로 불러와 사용하며, 매개변수를 받을 수 있어 유연한 스타일 생성이 가능합니다.
...
CSS Custom Properties는 "CSS 변수"로도 불리며, CSS 내에서 재사용 가능한 값을 정의할 수 있게 합니다. `--main-color: blue;`와 같이 `--` 접두사로 선언하고, `color: var(--main-color);`처럼 `var()` 함수로 참조합니다.
